...
/// The default-deny folder-rule manager (add rule + rules table). Shared between
/// the File Permissions tab and the Shell Access tab — both surfaces scope their
/// access to the same folder rules, so the exact same UI is rendered in two
/// places. Self-contained: owns its draft state and edits the shared store.
struct FilePermissionRulesSection: View {
      ...
      VStack(alignment: .leading, spacing: 14) {
        HStack(alignment: .top, spacing: 8) {
          Image(systemName: "lock.shield")
            ...
            .frame(width: 16)
            .padding(.top, 1)
          Text("Everything is denied by default. Folder rules grant read or write access to that folder and its children for both file editing and sandboxed shell; the most specific rule wins.")
            .font(.callout)
            .foregroundStyle(.secondary)
        }
